...
export default function DetailPage() {
  ...
  const [scooter, setScooter] = useState<any>(() => {
    if (typeof window !== 'undefined' && id) {
      const cached = clientCache.get<any>(`scooter_${id}`)
      if (cached?.scooter) return cached.scooter
      const catalog = clientCache.get<any>('catalog_data')
      if (catalog?.scooters) {
        const found = catalog.scooters.find((s: any) => s.id.toString() === id)
  ...
  const [vendor, setVendor] = useState<any>(() => {
    if (typeof window !== 'undefined' && id) {
      const cached = clientCache.get<any>(`scooter_${id}`)
      if (cached?.vendor) return cached.vendor
      const catalog = clientCache.get<any>('catalog_data')
      if (catalog?.vendors) {
        const sVendorId = scooter?.vendor_id || catalog?.scooters?.find((s: any) => s.id.toString() === id)?.vendor_id
        if (sVendorId) {
          return catalog.vendors.find((v: any) => v.id.toString() === sVendorId.toString()) || null
        }
      }
    }
    return null
  ...
  const [loading, setLoading] = useState<boolean>(() => !scooter)
